...
function test_poisson_asm_time_2(mesh)
    ##
    ## Test case solves the Poisson equation
    ##   u_xx + u_yy = 1,
    ## with the boundary condition
    ...
    ##
    N = size(mesh.p,2)
    @time S1 = @bilin_asm(a*(ux*vx+uy*vy),mesh,ones(N,1))
    @time S2 = @bilin_asm(ux*vx+uy*vy,mesh)
    #@time S2 = @bilin_asm_dispatch(ux*vx+uy*vy,mesh)
    #@time S2 = @bilin_asm(ux*vx+uy*vy,mesh)
    @time f = @lin_asm_dispatch(v,mesh,ones(N,1))
